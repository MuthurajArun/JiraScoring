...
app.on('message', async ({ send, stream, activity }) => {
  ...
  try {
    ...
    if (activity.text.toLowerCase().startsWith('/ac')) {
  const issueKey = activity.text.split(' ')[1];
  if (!issueKey) {
    stream.emit("Please provide a JIRA issue key like `/scoreDescription ABC-123`");
    return;
  }
...
console.log(fields);
    ...
    stream.emit(`Fetched description for issue ${issueKey}`);
      stream.emit(`<br>Acceptance Criteria :${description}`);
      const formatted = util
  .inspect(result, { depth: null })
  .replace(/\n/g, "<br>");

stream.emit((`Score value : ${formatted}`));
     // stream.emit(`<br>Score value : ${JSON.stringify(result, null, 2)}`);
    stream.emit(new MessageActivity().addAiGenerated().addFeedback());
    }
  } catch (err) {
    console.error(err);
